...
function PlayerAvatar({ player, size = 'md' }: { player: Player; size?: 'sm' | 'md' | 'lg' }) {
  ...
  const textClass = size === 'sm' ? 'text-2xl' : size === 'lg' ? 'text-5xl' : 'text-4xl';

  return (
    <Avatar className={`${sizeClass} border-2 border-white/10 bg-zinc-950 shadow-inner`}>
      <AvatarFallback className={`bg-zinc-950 ${textClass} font-bebas italic text-primary`}>
    ...
    <div className="space-y-6 lg:space-y-8 p-4 lg:p-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 lg:gap-6">
        <div className="space-y-1">
          <h2 className="text-4xl lg:text-7xl font-black uppercase tracking-tighter italic text-white flex items-center gap-3 lg:gap-4">
            <Users className="h-8 w-8 lg:h-14 lg:w-14 text-primary shrink-0" />
            PLANTILLA ÉLITE
          </h2>
          <p className="text-[9px] lg:text-xs font-black uppercase tracking-[0.25em] lg:tracking-[0.3em] text-primary/60 ml-1">
        ...
        <div className="relative w-full md:w-80">
          ...
          <Input
            ...
            className="pl-10 h-11 lg:h-12 bg-black/40 border-white/10 font-bold tracking-widest uppercase text-white"
            value={searchTerm}
      ...
      <div className="md:hidden space-y-2">
        {filteredPlayers?.map((player) => (
          <Link key={player.id} href={`/players/${player.id}`} className="block">
            <Card className="competition-card official-table-row border-white/5 bg-black/40 px-4 py-3 flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="font-black text-sm uppercase tracking-tight text-white truncate">{player.name}</h3>
              ...
              <div className="flex items-center gap-2 text-primary/50 shrink-0">
                {player.jerseyNumber != null && (
                  <span className="text-[8px] font-black uppercase tracking-widest">N° {player.jerseyNumber}</span>
                )}
